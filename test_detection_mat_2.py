...
#focntion assosicant N(bit) à la distance réelle
...
#linear approximate
# Reshape x and y into 2D arrays for use with scikit-learn
x = l_distance_real.reshape(-1, 1)
y = l_distances.reshape(-1, 1)
...
#test polynome obtenu

y_calc = []

for i in range(len(l_distances)):
    resultat = polynome(l_distances[i],coef[0])
    y_calc.append(resultat)

# Plot the original data points and the best-fit line
plt.plot(l_distance_real, l_distances,'o', label = "Initial data")
plt.plot(x, reg.predict(x_poly), 'ro', label='Best-fit line')
#plt.plot(x, model.predict(x.reshape(-1, 1)), 'b-', label='Best-fit curve')
#plt.plot(x,y_calc, 'g-', label='Polynomial function')
